...
#if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_USB) || \
         ...
         
{
        ...
        */
        if(0u != USB_GetRxBufferSize())
        {
            ...
            */
            do
            {
                oldDataCount = USB_GetRxBufferSize();
                CyDelay(USB_BYTE2BYTE_TIME_OUT);
            }
            while(USB_GetRxBufferSize() > oldDataCount);

            status = CYRET_SUCCESS;
            break;
        }
        /* If the data is not received, give a delay of 
        *  USB_BL_CHK_DELAY_MS and check again until the timeOut specified.
        */
        else
        {
            CyDelay(USB_BL_CHK_DELAY_MS);
        }
    }
    ...
    *count = 0u;
    ...
    while(USB_GetRxBufferSize() > 0u)
    {
        tempCount = USB_GetRxBufferSize();
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            for (iCntr = 0u; iCntr < tempCount; iCntr++)
            {
                ...
                pData[dataIndexCntr] = USB_ReadRxData();
                dataIndexCntr++;
            }

            /* Check if the last received byte is end of packet defined by bootloader
            *  If not wait for additional USB_WAIT_EOP_DELAY ms.
            */
            if(pData[dataIndexCntr - 1u] != USB_PACKET_EOP)
            {
                CyDelay(USB_WAIT_EOP_DELAY);
            }
        }
